...
germanpolls <- function(long = FALSE) {
  ...
  df <-
    map_df(institute, function(i) {
      ...
      table_head <-
        html_nodes(table, xpath = "//thead//th") %>% html_text()

      rdf[["datum"]] <-
        html_nodes(page, xpath = "//table//tbody//tr//td[1]") %>% html_text()
      ...
      for (head_el in seq_along(header)) {
        ...
        checkvec <-
          html_nodes(page, xpath = paste0("//table//tbody//tr//td[", hindex, "]")) %>% html_text()
